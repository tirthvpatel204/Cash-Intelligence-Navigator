...
import { registerWithEmail, loginWithEmail, loginAnonymously, logoutUser, loginWithGoogle } from '../lib/firebase';
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err?.code === 'auth/popup-blocked') {
        msg = 'Sign-in popup was blocked by browser. Please allow popups or use email.';
      } else if (err?.code === 'auth/popup-closed-by-user') {
        msg = 'Google sign-in popup was closed.';
      } else if (err?.code === 'auth/cancelled-popup-request') {
        msg = 'Google sign-in was cancelled.';
      } else if (err?.code === 'auth/unauthorized-domain') {
        msg = 'Domain is pending Google OAuth registration. You can sign in using email or Guest session.';
      }
      setError(msg);
    } finally {
